...
void ofApp::setupData( string filename="" )
{
    ...
    for (int i = 0; i < numClients; i++)
    {
        ...
        addClient(i,ip,port,name,r,m,s,live,vrt,hier,modeFlags);
        data.popTag();
    }
...
void ofApp::addClient(int i,string ip,int p,string n,bool r,bool m,bool s, bool live,bool vrt, bool hierarchy, int modeFlags)
{
    ...
    if(uniqueClient){
        client *c = new client(i,ip,p,n,r,m,s,live,vrt, hierarchy, modeFlags);
...
void ofApp::doGui() {
    ...
    if (this->guiVisible)
    {
        ...
        if ( ImGui::CollapsingHeader("Global Settings", NULL, ImGuiTreeNodeFlags_DefaultOpen) )
            ...
            {
                addClient(clients.size(), ofToString(client_ip), client_port, ofToString(client_name), false, false, false, false, true, false, 0);
